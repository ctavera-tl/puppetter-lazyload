...
const argv = require('yargs')
  .options({
    ...
    useBot: {
      alias: 'b',
      describe: 'Use Bot crawler user agent',
      default: false,
    },
    save: {
      alias: 's',
    ...
    scroll: {
      describe:
        'Filename for screenshot of scrolled page. Only worked with --save option.',
      default: PNG_SCROLL_FILENAME,
    },
    noscroll: {
      describe:
        'Filename for screenshot of non-scrolled page. Only worked with --save option.',
...
(async () => {
  const browser = await puppeteer.launch({
    // headless: false,
    defaultViewport: argv.useMobile ? MOBILE_VIEWPORT : DEFAULT_VIEWPORT,
  });
  ...
  async function screenshotPageWithoutScroll(url) {
    ...
    const viewport = Object.assign(
      {},
      argv.useMobile ? MOBILE_VIEWPORT : DEFAULT_VIEWPORT
    );
  ...
  async function screenshotPageAfterScroll(url) {
    ...
    // const maxScrollHeight = await page.evaluate('document.scrollingElement.scrollHeight');
  ...
  const { png: pngDiff, buffer: diffBuffer } = await resizeImage(
    fs.readFileSync(argv.diff),
    0.25
  );
  ...
  await page.setContent(`
     ...
     <html>
       ...
       <body>
         ...
         <h2 class="check">Site result: <span class="${
           passed ? 'passed' : 'failed'
         }">${passed ? 'PASSED' : 'FAILED'}</span></h2>
         ...
         <section>
           <div>
             ...
             <img src="data:img/png;base64,${screenshotA.toString(
               'base64'
             )}" class="screenshot">
           </div>
           <div>
             ...
             <img src="data:img/png;base64,${diffBuffer.toString(
               'base64'
             )}" class="screenshot">
           </div>
           <div>
             ...
             <img src="data:img/png;base64,${screenshotB.toString(
               'base64'
             )}" class="screenshot">
           </div>
         </section>
